...
# Per-set filter presets (radio effects etc.) applied between the clean source
# WAV and the encoded webm — see build-tools/audio-presets.conf
# shellcheck source=build-tools/audio-presets.sh
source build-tools/audio-presets.sh
...
for file in "${files[@]}"; do
    ...
    preset=$(audio_preset_for_file "$file")
    ok=0
    if [ -n "$preset" ]; then
        echo "Processing: $rel_path (preset: $preset)"
        chain=$(audio_preset_filter "$preset" "$file") \
            && ffmpeg -i "$file" -filter_complex "${chain};[out]loudnorm=I=${target_lufs}:TP=${target_tp}:LRA=${target_lra}[norm]" -map "[norm]" -c:a libopus -b:a ${bitrate} -ar 48000 "$output_file" -y -loglevel warning 2>/dev/null \
            || ok=1
    else
        echo "Processing: $rel_path"
        ffmpeg -i "$file" -af loudnorm=I=${target_lufs}:TP=${target_tp}:LRA=${target_lra} -c:a libopus -b:a ${bitrate} -ar 48000 "$output_file" -y -loglevel warning 2>/dev/null \
            || ok=1
    fi
