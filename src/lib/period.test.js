...
describe('Period.completeAtDuration', () => {
    it('happy path: elapsed set to duration, remaining set to 0', () => {
        const period = makePeriod({
            duration: 24 * 60 * 1000,
            elapsed: 24 * 60 * 1000,
            remaining: 0,
            userIntendedDuration: 24 * 60 * 1000,
        })
        const result = Period.completeAtDuration(period)

        expect(result.state.elapsed).toBe(24 * 60 * 1000)
        expect(result.state.remaining).toBe(0)
    })

    it('overshoot elapsed is clamped down to duration (not preserved)', () => {
        const period = makePeriod({
            duration: 24 * 60 * 1000,
            elapsed: 30 * 60 * 1000, // overshoot past duration
            remaining: 0,
            userIntendedDuration: 24 * 60 * 1000,
        })
        const result = Period.completeAtDuration(period)
    ...
    it('state.duration is unchanged', () => {
        const period = makePeriod({
            duration: 24 * 60 * 1000,
            elapsed: 30 * 60 * 1000,
            remaining: 0,
    ...
    it('config.userIntendedDuration is unchanged', () => {
        const period = makePeriod({
            duration: 24 * 60 * 1000,
            elapsed: 30 * 60 * 1000,
            userIntendedDuration: 24 * 60 * 1000,
    ...
    it('input period is not mutated', () => {
        const period = makePeriod({ duration: 24 * 60 * 1000, elapsed: 30 * 60 * 1000 })
        const originalElapsed = period.state.elapsed

        Period.completeAtDuration(period)
