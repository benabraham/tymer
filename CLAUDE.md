...
- **Applying the update** — `src/lib/app-update.js` owns the policy. `registerType: 'autoUpdate'` would reload unconditionally; instead `onNeedReload` sets the `updateReady` signal and an `effect` reloads as soon as it is safe: idle or completed, and the durations panel closed. Reloading is lossless (the session is persisted and elapsed is clock-derived) but it would drop half-typed editor text. While a session is running the `BuildInfo` avatar pulses (opacity only — its box never changes, so nothing shifts) and an "Update available · Reload now" notice appears next to it: below the avatar on desktop, as a bottom-right toast under 800px where the controls row owns the top corner. Otherwise the reload happens when the session ends.
