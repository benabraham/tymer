...
- While anchored, elapsed can't be manually adjusted (`canAdjustElapsed*` guards) and periods with a successor never auto-extend at a boundary — `advanceOverduePeriods` auto-advances (or catches up through multiple overdue boundaries) each tick instead. The LAST period is the exception: it auto-extends like normal, so an anchored session never self-finishes and no wall-clock time since the anchor is lost, even when the configured durations are shorter than the time to fill (the user ends it via Finish). Consequently, any past `@` time is valid in the live editor (only a future one is ignored), and `resumeTimer` reconciles+catches up when a paused session is anchored (reachable by typing `@h:mm` while paused).
